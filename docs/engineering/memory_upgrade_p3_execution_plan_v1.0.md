...
- 当前最小规则已经成立：
  - 无 namespace 提示时，仍默认：
    - `companion`
  - `project` primary namespace 下，会切到：
    - `project_ops`
...
- `memory-upgrade-harness.ts` 当前也已开始显式校验：
  - 默认路径仍是 `companion`
  - `project` primary namespace 下，active pack 会切到 `project_ops`

当前已成立的第二刀代码事实：

- `apps/web/lib/chat/memory-packs.ts` 当前已开始让 `resolveActiveScenarioMemoryPack(...)` 同时考虑：
...
- 也就是说，pack selection 当前已经不只是 namespace-driven，而开始变成 `namespace + knowledge` 的联合判断
- `memory-upgrade-harness.ts` 当前也已开始显式校验：
  - world-primary namespace 下，project-scoped knowledge 仍可触发 `project_knowledge_priority`

当前已成立的第三刀代码事实：

- `apps/web/lib/chat/memory-knowledge.ts` 当前已开始让 knowledge prompt budget 受 `activePackId` 影响
- 当前最小规则已经成立：
  - `companion` 继续使用更紧的 knowledge budget，只保留 `project / world`
  - `project_ops` 在 `project / world` 优先的前提下，可在预算允许时额外带入一条 `general` knowledge
- `apps/web/lib/chat/runtime.ts` 当前也已开始把 active scenario pack 传入 knowledge prompt section，使 pack 选择不只影响 pack metadata / prompt 标题，也开始真实影响 knowledge 消费预算
- `memory-upgrade-harness.ts` 当前也已开始显式校验：
