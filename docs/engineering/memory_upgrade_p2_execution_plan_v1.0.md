...
- `packages/core/memory/knowledge.ts` 当前已新增首版：
  - `KnowledgeResource`
  - `KnowledgeSnapshot`
  - `KnowledgeLink`
- `apps/web/lib/chat/memory-knowledge.ts` 当前已新增：
  - `buildKnowledgeSnapshot(...)`
  - `buildRuntimeKnowledgeSnippet(...)`
  - `buildKnowledgePromptSection(...)`
  - `buildKnowledgeSummary(...)`
- runtime `buildAgentSystemPrompt(...)` 当前也已开始支持最小 `Knowledge Layer` 注入段
- assistant metadata / debug metadata 当前也已开始写入最小 knowledge 摘要：
  - `count`
  - `titles`
  - `source_kinds`
...
- `packages/core/memory/compaction.ts` 当前已新增首版：
  - `CompactedThreadSummary`
  - `CompactedThreadLifecycleStatus`
  - `CompactedThreadContinuityStatus`
- `apps/web/lib/chat/thread-compaction.ts` 当前已新增：
...
- assistant metadata / debug metadata 当前也已开始暴露最小 thread compaction 摘要：
  ...
  - `lifecycle_status`
  - `continuity_status`
- `memory-upgrade-harness.ts` 当前也已开始显式校验 compaction seam：
  - metadata reader
  - prompt 注入
  - focus context 可见性
