...
- [compaction.ts](/Users/caoq/git/sparkcore/packages/core/memory/compaction.ts) 当前已开始把 lifecycle policy 从 `policy id + survival mode` 继续推进成更明确的 retention decision contract：
  - `retention_decision_group`
  - `survival_rationale`
- [thread-compaction.ts](/Users/caoq/git/sparkcore/apps/web/lib/chat/thread-compaction.ts) 当前已开始让 keep/drop decision 不再只依赖零散条件，而开始优先复用：
  - `retention_decision_group`
  - `survival_rationale`
- 当前最小规则已经成立：
  - `focus_anchor` 下：
    - `retention_decision_group = anchor_preserve`
    - `survival_rationale = focus_anchor_survives`
  - `closed + minimal` 下：
    - `retention_decision_group = closed_decay_prune`
    - keep/drop 会优先按这层 group 直接判成 prune
- assistant metadata / runtime debug metadata / [memory-upgrade-harness.ts](/Users/caoq/git/sparkcore/apps/web/scripts/memory-upgrade-harness.ts) 当前也已同步接上这层 retention lifecycle grouping
...
- [knowledge.ts](/Users/caoq/git/sparkcore/packages/core/memory/knowledge.ts) 当前已开始把 knowledge weighting 从 `scope / source kind` 上提成更明确的 governance contract：
  - `KnowledgeGovernanceClass`
  - `authoritative`
  - `contextual`
  - `reference`
- [memory-knowledge.ts](/Users/caoq/git/sparkcore/apps/web/lib/chat/memory-knowledge.ts) 当前已开始让 `buildKnowledgeRouteWeighting(...)` 不只输出：
  - `scope_weight`
  - `namespace_weight`
  - `pack_weight`
  ...
  - `governance_weight`
- 当前最小规则已经成立：
  - `project_document -> authoritative`
  - `workspace_note -> contextual`
  - `external_reference -> reference`
- `selectKnowledgeForPrompt(...)` 当前也已开始真实复用这层 governance weighting，而不再只按 scope / namespace / pack 做排序
- assistant metadata / runtime debug metadata / [memory-upgrade-harness.ts](/Users/caoq/git/sparkcore/apps/web/scripts/memory-upgrade-harness.ts) 当前也已同步接上：
  - `knowledge_governance_classes`
  - `governance_counts`

当前已成立的第二刀代码事实：

- [memory-packs.ts](/Users/caoq/git/sparkcore/apps/web/lib/chat/memory-packs.ts) 当前已开始让 knowledge governance weighting 不只影响 snippet selection，还开始进入更明确的 route/budget orchestration 输出：
  - `governance_route_bias`
- 当前最小规则已经成立：
  - `project_ops + project knowledge` 下：
    - `governance_route_bias = authoritative`
    - `knowledge_budget_weight` 当前会从 `0.9` 上调到 `0.95`
- 也就是说，knowledge governance 当前已经不只是在 `memory-knowledge.ts` 内部排序，而开始真实影响：
  - active scenario pack 的 route/budget 输出
  - assistant metadata
  - runtime debug metadata
  - prompt 文案
  - harness
...
- [packs.ts](/Users/caoq/git/sparkcore/packages/core/memory/packs.ts) 当前已开始把 scenario orchestration 从 strategy bundle 继续上提成正式 policy contract：
  - `ScenarioStrategyPolicyId`
  - `ScenarioOrchestrationMode`
- [memory-packs.ts](/Users/caoq/git/sparkcore/apps/web/lib/chat/memory-packs.ts) 当前已开始提供：
  - `resolveScenarioMemoryPackPolicy(...)`
  并让 active scenario pack 不只输出：
  - `strategy_bundle_id`
...
- 当前最小规则已经成立：
  - `project_ops` 下：
    - `strategy_policy_id = project_delivery_policy`
    - `orchestration_mode = execution_centered`
- 这层 strategy policy 当前也已进入：
  - prompt 文案
...
- `P6-2`
  ...
  - 当前已开始从：
    - `retention_policy_id`
    - `cross_layer_survival_mode`
    这层 lifecycle policy contract 进入真实主路径
  - 当前也已继续推进到：
    - `retention_decision_group`
    - `survival_rationale`
  - 这层 policy 当前也已进入：
...
- `P6-3`
  ...
  - 当前已开始从：
    ...
    - `governance_class`
    - `governance_weight`
    这层 governance contract 进入真实 knowledge weighting 主路径
  - 当前也已继续推进到：
    - `governance_route_bias`
...
- `P6-4`
  ...
  - 当前已开始从：
    - `ScenarioStrategyPolicyId`
    - `ScenarioOrchestrationMode`
    这层 strategy policy contract 进入真实 scenario pack 主路径
  - 当前也已继续推进到：
    - `strategy_policy_id`
    - `orchestration_mode`
    - `governance_route_bias`
    之间更明确的 orchestration 协调面
  - 这层 policy 当前也已进入：
...
- `P6-5`
  - 第一版已成立
  - `memory-upgrade-harness.ts` 当前已开始显式产出 `p6_regression_gate`
- 当前第一版 `P6` gate 已开始锁：
...
- **继续补 `P6-4 / P6-5` 的结合面**
- 当前 `P6-1 ~ P6-5` 已经不再缺少阶段 gate，下一步更值的是把 scenario strategy orchestration 再往阶段级一致性和治理面补一刀，而不是重新发散回单点能力补洞
