...
2. 通过 preparation 层加载：
   - `SessionContext`
   - `RoleCorePacket`
   - `RuntimeMemoryContext`
3. 形成 `PreparedRuntimeTurn`
4. 通过 `runPreparedRuntimeTurn(prepared)` 执行本轮运行
5. 解析结果并形成标准输出
6. 触发必要的记忆写入
7. 触发必要的定时任务 / follow-up 请求
8. 返回 `RuntimeTurnResult`
...
- `prepareRuntimeTurn(...)` 已存在
- `runPreparedRuntimeTurn(prepared)` 已存在第一版执行薄壳
