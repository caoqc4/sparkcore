...
└─────────────────────┬───────────────────────────────┘
        ...
        ┌─────────────▼─────────────┐
        │   输出治理层               │  ← 定义如何表达回复
        │   RoleExpressionPacket    │    （身份、语气、表达原则）
        │   RelationshipPacket      │    （关系阶段、风格、即时覆写）
        │   SceneDeliveryPacket     │    （模态、长度、语言）
        │   KnowledgeBriefPacket    │    （知识注入规则）
        └─────────────┬─────────────┘
                      │
        ┌─────────────▼─────────────┐
        │   LLM 生成                 │  ← 文本 / 图片 / 音频
        └─────────────┬─────────────┘
                      │
        ┌─────────────▼─────────────┐
        │   真人感输出层             │  ← 将原始输出转化为自然的人类语感
        │   姿态 · 节奏              │    20+ 维度：开场风格、语气张力、
        │   开场 · 跟进              │    情绪复现、跟进深度等
        └─────────────┬─────────────┘
                      │
        ┌─────────────▼─────────────┐
...
**输出治理层** 控制 Agent *表达什么*——将记忆和调度的决策转化为四个治理包，约束并引导 LLM 生成。见 [`apps/web/lib/chat/output-governance.ts`](./apps/web/lib/chat/output-governance.ts)。

**真人感输出层** 控制*怎么表达*——对 LLM 原始输出进行后处理，在 20+ 个维度上校准自然度（姿态、节奏、开场风格、语气张力、跟进深度、情绪复现等）。见 [`apps/web/lib/chat/humanized-delivery-strategy.ts`](./apps/web/lib/chat/humanized-delivery-strategy.ts)。

**反馈 / 跟进层** 实现 Agent 的主动行为——基于对话状态调度温和的跟进消息和主动触达。见 [`apps/web/lib/chat/follow-up-executor.ts`](./apps/web/lib/chat/follow-up-executor.ts)。
