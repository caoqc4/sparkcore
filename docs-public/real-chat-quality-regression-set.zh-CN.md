...
这份样例集刻意保持轻量。它不是重型评测平台，而是针对当前 `/chat` 工作台的一组可重复执行的真实对话质量基线。
...
### 1. 同一个 agent 的昵称 / 称呼偏好在新 thread 中仍然连续

- 优先级：`P0`
- 类别：`thread`
- 目标：确认 relationship 连续性可以跨同一个 agent 的新 thread，而不是只能在旧线程里勉强生效
...
- 优先级：`P0`
- 类别：`fidelity`
- 目标：确认命中的 profession 记忆会被直接说出来，而不是被模糊化或说歪
...
- 最终回答直接说出 `product designer`
- 不再把“没有对话历史”和“没有长期记忆”混为一谈

### 3. 回复优先跟随当前用户最后一条消息语言，而不是漂移

- 优先级：`P0`
- 类别：`language`
- 目标：确认当前轮用户消息的主语言拥有最高优先级

步骤：

1. 先发送一条英文消息，例如：`Please introduce yourself briefly.`
2. 再发送一条中文消息，例如：`你记得我做什么工作吗？`
3. 展开后一个回复的 runtime summary
...
### 4. relationship 风格在同一线程内多轮保持可感知连续

- 优先级：`P0`
- 类别：`fidelity`
- 目标：确认 relationship 风格不只是“被记住”，而是在同一线程内持续“被演出来”
...
### 5. incorrect / restore 会稳定改变后续 recall 资格

- 优先级：`P0`
- 类别：`correction`
- 目标：确认纠错会影响后续真实对话行为，而不只是 memory 行本身的状态变化
